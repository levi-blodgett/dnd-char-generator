...
function format_secondary_dropdowns(list, listItems, checkbox) {
  ...
  let listOfSubracesInDropdown = document.querySelectorAll(
...
function on_click_decider(_id, _class) {
  ...
  if (isCheckboxChecked) {
    ...
    if (input_class === "race_class") {
      let lengthOfArray_2 = document.querySelectorAll(
...
function on_subrace_dropdown_option_click(e) {
  let targets_ul = document.getElementById(
...
const add_events_for_dropdowns = (
...
function on_dropdown_option_click(_id, _class) {
  ...
  if (document.getElementById(ID).checked) {
    ...
    let lengthOfArray = document.querySelectorAll(
      `input.${input_class}`
    ).length;
